...
def test_delete_profile_stops_running(app_client: TestClient):
    ...
    with patch.object(main.browser_mgr, "stop", AsyncMock()) as mock_stop:
        resp = app_client.delete(f"/api/profiles/{pid}")
    assert resp.status_code == 200
    mock_stop.assert_called_once_with(pid)
...
def test_system_status(app_client: TestClient):
    ...
    assert "is_desktop" in data
    assert isinstance(data["is_desktop"], bool)
...
def test_skip_cleanup_endpoint(app_client: TestClient):
    """POST /api/profiles/skip-cleanup should set browser_mgr.skip_cleanup to True."""
    # Reset skip_cleanup flag
    main.browser_mgr.skip_cleanup = False
    
    resp = app_client.post("/api/profiles/skip-cleanup")
    assert resp.status_code == 200
    assert resp.json() == {"ok": True}
    assert main.browser_mgr.skip_cleanup is True
