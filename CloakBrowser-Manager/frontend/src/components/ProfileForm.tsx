...
export function ProfileForm({ profile, proxies, onSave, onDelete, onCancel }: ProfileFormProps) {
  ...
  const handleDelete = async () => {
    if (!onDelete) return;
    if (!confirm("Xóa profile này? Toàn bộ dữ liệu trình duyệt sẽ bị xóa vĩnh viễn và không thể khôi phục.")) return;
    setDeleting(true);
    try {
      await onDelete();
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 max-w-2xl mx-auto w-full h-full overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="submit" disabled={saving} className="btn-primary flex items-center gap-1.5">
            ...
            <span>{saving ? "Đang lưu..." : isEdit ? "Lưu" : "Tạo mới"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="label">Tên Profile</label>
              <input
                className="input"
                value={form.name}
                onChange={(e) => set("name", e.target.value)}
                placeholder="Ví dụ: Amazon Seller #1"
                required
              />
            </div>
            <div>
        ...
        {/* Network */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Mạng & Proxy</h3>
          <div className="space-y-3">
            <div>
              <label className="label">Proxy</label>
              <select
                className="input"
                value={form.proxy_id || "none"}
                onChange={(e) => {
                  const val = e.target.value;
                  set("proxy_id", val === "none" ? null : val);
                  // clear the old string proxy if they select a proper proxy
                  if (val !== "none") {
                     set("proxy", null);
                     if (form.geoip) {
                       const selectedProxy = proxies.find(p => p.id === val);
                       if (selectedProxy) {
                         if (selectedProxy.timezone) set("timezone", selectedProxy.timezone);
                         if (selectedProxy.locale) set("locale", selectedProxy.locale);
                       }
                     }
                  }
                }}
              >
                <option value="none">Không dùng Proxy (Kết nối trực tiếp)</option>
                {proxies.map(p => (
              ...
              {form.proxy && !form.proxy_id && (
                <div className="mt-2 flex items-center gap-2">
                  <input
                    className="input text-xs text-gray-500 bg-surface-2 cursor-not-allowed"
                    value={form.proxy}
                    disabled
                    title="Chuỗi proxy cũ. Chọn proxy từ danh sách trên để di chuyển cấu hình."
                  />
                  <span className="text-[10px] text-amber-500">Cấu hình cũ (Chỉ đọc)</span>
                </div>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Múi giờ (Timezone)</label>
                <input
                  className="input"
                  value={form.timezone ?? ""}
                  onChange={(e) => set("timezone", e.target.value || null)}
                  placeholder="Ví dụ: America/New_York"
                />
              </div>
              <div>
                <label className="label">Ngôn ngữ (Locale)</label>
                <input
                  className="input"
                  value={form.locale ?? ""}
                  onChange={(e) => set("locale", e.target.value || null)}
                  placeholder="Ví dụ: en-US"
                />
              </div>
            </div>
            <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  set("geoip", checked);
                  if (checked && form.proxy_id) {
                    const selectedProxy = proxies.find(p => p.id === form.proxy_id);
                    if (selectedProxy) {
                      if (selectedProxy.timezone) set("timezone", selectedProxy.timezone);
                      if (selectedProxy.locale) set("locale", selectedProxy.locale);
                    }
        ...
        {/* Hardware */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Phần cứng & Vân tay</h3>
          <div className="space-y-3">
            <div>
              <label className="label">Độ phân giải màn hình</label>
              <select
                className="input"
            ...
            {currentResolution === "custom" && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="label">Chiều rộng</label>
                  <input
                    className="input"
                    type="number"
            ...
            <div>
              <label className="label">Số luồng CPU (Hardware Concurrency)</label>
              <input
                className="input"
                type="number"
                value={form.hardware_concurrency ?? ""}
                onChange={(e) => set("hardware_concurrency", e.target.value ? Number(e.target.value) : null)}
                placeholder="Tự động (tạo từ Seed)"
              />
            </div>
            <div>
              <label className="label">Cấu hình GPU mẫu</label>
              <select
                className="input"
                value=""
                onChange={(e) => {
              ...
              >
                <option value="">Chọn mẫu GPU có sẵn...</option>
            ...
            <div>
              <label className="label">Nhà sản xuất GPU (Vendor)</label>
              <input
                className="input"
            ...
            <div>
              <label className="label">Bộ dựng hình GPU (Renderer)</label>
              <input
                className="input"
        ...
        {/* Behavior */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Hành vi người dùng</h3>
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
              <input
                type="checkbox"
                ...
                className="rounded border-border bg-surface-2"
              />
              Mô phỏng hành vi chuột, bàn phím và cuộn giống người thật
            </label>
            {form.humanize && (
              <div>
                <label className="label">Mức độ mô phỏng</label>
                <select
                  className="input"
                ...
                >
                  ...
                  <option value="careful">Cẩn thận (chậm và chính xác hơn)</option>
                </select>
              </div>
            )}
            <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
              <input
                ...
                className="rounded border-border bg-surface-2"
              />
              Bật tự động đồng bộ Clipboard trong cửa sổ VNC
            </label>
            <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
              <input
                type="checkbox"
                ...
                className="rounded border-border bg-surface-2"
              />
              Tự động khởi chạy khi khởi động Container
            </label>
            <div>
              <label className="label">Chế độ giao diện (Color Scheme)</label>
              <select
                className="input"
              ...
              >
                ...
                <option value="no-preference">Không ưu tiên (No preference)</option>
              </select>
            </div>
            <div>
              <label className="label">Chuỗi User Agent</label>
              <input
                className="input"
                value={form.user_agent ?? ""}
                onChange={(e) => set("user_agent", e.target.value || null)}
                placeholder="Tự động (từ nhân Chromium)"
              />
            </div>
          </div>
        </section>
        ...
        {/* Tags */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Thẻ tag</h3>
          {(form.tags ?? []).length > 0 && (
        ...
        {/* Launch Args */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Tham số Chromium</h3>
          <p className="text-xs text-gray-500 mb-2">Các tham số Chromium tùy chỉnh khi khởi chạy (Ví dụ: --load-extension, --disable-features)</p>
        ...
        {/* Notes */}
        <section>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Ghi chú</h3>
          <textarea
            className="input min-h-[80px] resize-y"
            value={form.notes ?? ""}
            onChange={(e) => set("notes", e.target.value || null)}
            placeholder="Ghi chú thêm về profile này (tùy chọn)..."
          />
        </section>
      </div>
